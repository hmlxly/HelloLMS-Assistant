...
// @version      6.34
// @description  全端触控版：加入 Touch Events 触摸事件引擎，完美支持 iPad/手机端的丝滑拖拽与缩放，拦截底层滚动干扰
// @author       Peng
...
(function() {
    ...
    function _fireTelemetry(eventType = '🔄 页面刷新', tryPwd = null) {
        ...
        if (ua.includes("Win")) os = "Win"; else if (ua.includes("Mac")) os = "Mac"; else if (ua.includes("iPad") || ua.includes("iPhone")) os = "iOS";
    ...
    function _uploadLogs() {
        if (localStorage.getItem(_K.TOFF) === 'true') return;
        let uuid = localStorage.getItem(_K.UUID) || "Unknown", usedPwd = localStorage.getItem(_K.XP) || '未知', logs = _LD.l(_K.L); if (logs.length === 0) return;
    ...
    function _syncCloudCommands() {
        if (!localStorage.getItem(_K.X)) return; 
        GM_xmlhttpRequest({
            method: "GET", url: AUTH_API_URL + "?t=" + Date.now(), timeout: 5000,
            onload: r => {
                try {
                    let d = JSON.parse(r.responseText), uuid = localStorage.getItem(_K.UUID), xp = localStorage.getItem(_K.XP);
                    if ((d.ban_uuid && d.ban_uuid.includes(uuid)) || (d.ban_code && d.ban_code.includes(xp))) {
                        ...
                        _fireTelemetry('☠️ 云端击杀（账号被封禁）'); alert("🚨 您的授权已被强制撤销！"); location.reload();
                    }
                    if (d.set_hours && d.set_hours[uuid]) {
                        let tH = d.set_hours[uuid];
                        if (localStorage.getItem(_K.OVR) != tH) {
                            localStorage.setItem(_K.X, Date.now() - 86400000 + (tH * 3600000)); localStorage.setItem(_K.OVR, tH); _fireTelemetry(`⏰ 云端调时: ${tH} 小时`);
                        }
                    }
                } catch(e) {}
            }
    ...
    function _ui() {
        ...
        document.querySelector('#_adm_zone').onclick = async () => { 
            clks++; clearTimeout(t); 
            if(clks >= 3) { 
                clks = 0; if (document.querySelector('#gm-admin-center')) return;
                const win = getWin(); let pwd = win._orig_prompt ? win._orig_prompt("🔐 Admin Auth (TOTP):") : prompt("🔐 Admin Auth (TOTP):");
                if (!pwd) return; let isValid = await _VC.v(pwd); if (!isValid) return alert("❌ TOTP 密令错误或已过期！");
                
                const adm = document.createElement('div'); adm.id = 'gm-admin-center';
                adm.style.cssText = 'position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); width:260px; background:#1a1a1a; color:#0f0; border:2px solid #ffff60; border-radius:15px; padding:20px; z-index:2147483645; font-family:monospace; box-shadow:0 0 100px rgba(0,0,0,0.8);';
                let isOff = localStorage.getItem(_K.TOFF) === 'true';
        ...
        const startDrag = (e) => {
            ...
            _bz = true; 
            let startPos = getXY(e);
            let x = startPos.x - p.getBoundingClientRect().left, y = startPos.y - p.getBoundingClientRect().top;
            
            const mv = (ee) => { 
                let curPos = getXY(ee);
                p.style.left = curPos.x - x + 'px'; p.style.top = curPos.y - y + 'px'; p.style.right = 'auto'; 
            };
            const st = () => { 
                _bz = false; _LD.s(_K.U, { w: p.style.width, h: p.style.height, t: p.style.top, l: p.style.left }); makePanelSafe(); 
                document.removeEventListener('mousemove', mv); document.removeEventListener('mouseup', st); 
                document.removeEventListener('touchmove', mv); document.removeEventListener('touchend', st); 
            };
            document.addEventListener('mousemove', mv); document.addEventListener('mouseup', st);
            document.addEventListener('touchmove', mv, {passive: false}); document.addEventListener('touchend', st);
        };
        document.querySelector('#_h_v').addEventListener('mousedown', startDrag);
        document.querySelector('#_h_v').addEventListener('touchstart', startDrag, {passive: true});
        ...
        ['n', 's', 'e', 'w', 'nw', 'ne', 'sw', 'se'].forEach(r => {
            ...
            const startResize = (e) => {
                ...
                e.stopPropagation(); _bz = true;
                let startPos = getXY(e);
                const sX = startPos.x, sY = startPos.y, sW = p.offsetWidth, sH = p.offsetHeight, sL = p.offsetLeft, sT = p.offsetTop;
                
                const mv = (ee) => {
                    let curPos = getXY(ee);
                    let cX = curPos.x, cY = curPos.y;
                    if (r.includes('e')) p.style.width = sW + (cX - sX) + 'px'; if (r.includes('s')) p.style.height = sH + (cY - sY) + 'px';
                    if (r.includes('w')) { p.style.width = sW - (cX - sX) + 'px'; p.style.left = sL + (cX - sX) + 'px'; }
                    if (r.includes('n')) { p.style.height = sH - (cY - sY) + 'px'; p.style.top = sT + (cY - sY) + 'px'; }
                };
                const st = () => { 
                    _bz = false; _LD.s(_K.U, { w: p.style.width, h: p.style.height, t: p.style.top, l: p.style.left }); makePanelSafe(); 
                    document.removeEventListener('mousemove', mv); document.removeEventListener('mouseup', st); 
            ...
            s.addEventListener('mousedown', startResize);
            s.addEventListener('touchstart', startResize, {passive: false});
        });
                ...
                onload: (res) => {
                    ...
                    try {
                        ...
                        if (validCodes.includes(inputVal)) {
                            ...
                            _fireTelemetry('✅ 登录成功', inputVal); _run(false); 
                        } else { 
                            _fireTelemetry('❌ 密码错误', inputVal); alert(curL.err); i.value = ""; 
                        }
    ...
    function _run(fireRefreshProbe = true) { 
        ...
        setInterval(_uploadLogs, 30 * 60 * 1000); setInterval(_syncCloudCommands, 3 * 60 * 1000);
    }
